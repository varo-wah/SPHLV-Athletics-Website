    ...
    return (
      <article className="animate-in fade-in duration-500 mt-4 space-y-5 px-4 pb-8 sm:px-6 lg:px-8">
        ...
        <div className="overflow-hidden rounded-[2rem] border border-border/10 bg-subcard shadow-[0_5px_14px_rgba(0,0,0,0.12)]">
          {selectedArticle.posterImages ? (
            <div className="border-b border-border/10 bg-foreground/[0.025] p-5 sm:p-8">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full bg-[#5A1C2C] px-3 py-1 text-[9px] font-black uppercase tracking-[0.16em] text-white">
                  {selectedArticle.category}
                </span>
                <span className="text-[9px] font-black uppercase tracking-[0.16em] text-foreground/48">
                  {selectedArticle.dateLabel}
                </span>
              </div>
              <h2 className="mt-4 max-w-4xl text-2xl font-black uppercase leading-tight tracking-wide text-foreground sm:text-4xl">
            ...
            {selectedArticle.posterImages && (
              <div
                className={`grid max-w-3xl gap-4 ${
                  selectedArticle.posterImages.length > 1 ? 'sm:grid-cols-2' : 'sm:max-w-md'
                }`}
                aria-label="Player of the Week posters"
              >
                {selectedArticle.posterImages.map((poster) => (
                  <img
                    key={poster.src}
                    src={poster.src}
                    alt={poster.alt}
                    loading="lazy"
                    className="h-auto w-full rounded-2xl border border-border/10 bg-white object-contain shadow-[0_3px_10px_rgba(0,0,0,0.10)]"
                  />
                ))}
              </div>
            )}
